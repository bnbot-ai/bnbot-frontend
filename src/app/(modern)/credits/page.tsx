...
const Credits = () => {
  ...
  // Fetch user credits from localStorage
  useEffect(() => {
    try {
  ...
  const fetchPaymentHistory = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/api/v1/payments/orders`,
        {
          headers: {
  ...
  const handlePurchase = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/api/v1/payments/create-payment`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${accessToken}`,
          ...
          body: JSON.stringify({
            ...
            redirect_url: `${window.location.origin}/credits`,
  ...
  const refreshCredits = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/api/v1/payments/credits`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            'Content-Type': 'application/json',
      ...
      if (userData.credits !== undefined) {
        ...
        // Update localStorage
        const existingData = localStorage.getItem('userData.xid');
        if (existingData) {
          const parsedData = JSON.parse(existingData);
          parsedData.credits = userData.credits;
          localStorage.setItem('userData.xid', JSON.stringify(parsedData));
        } else {
          localStorage.setItem('userData.xid', JSON.stringify(userData));
        }
      }
    } catch (error) {
        ...
        <div className="mt-3 md:mt-4 flex items-center justify-center text-3xl md:text-4xl font-semibold">
          ...
          <div className="group relative ml-2">
            ...
            <div className="absolute left-1/2 transform -translate-x-1/2 bottom-full mb-2 md:top-full md:bottom-auto md:mb-0 md:mt-2 z-10 hidden w-60 md:w-72 rounded-md border border-gray-200 bg-white p-2 text-left text-xs font-medium text-black shadow-lg group-hover:block">
              Credits are used for AI features and can be purchased with
              cryptocurrency.
            </div>
          </div>
        </div>
