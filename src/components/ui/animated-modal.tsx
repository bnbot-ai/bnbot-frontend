...
export const ModalProvider = ({ children }: { children: ReactNode }) => {
  ...
  const showModal = (
    ...
    setModalTweet(tweet);
    setModalType(type);
    if (tweetLink) setTweetLink(tweetLink);
    setOpen(true);
  };

  const isTrendTweet = (tweet: TrendTweet | Tweet | null): tweet is TrendTweet => {
    return tweet !== null && 'id_str' in tweet;
  };
  ...
  return (
    <ModalContext.Provider value={{ 
      ...
      { modalType === 'boost' ? (
        <BoostModal
          asset={modalContent}
          initialTweet={modalTweet as Tweet}
        />
      ) : (
        <TweetDetail 
...
export const ModalBody = ({
  ...
  const handleClose = () => {
    ...
    // 添加一个标志或使用 setTimeout 来防止立即触发新的点击事件
    document.body.style.pointerEvents = 'none';
    setTimeout(() => {
      document.body.style.pointerEvents = 'auto';
    }, 300); // 300ms 应该足够让动画完成并防止误触
  };
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ref={modalRef}
            className={cn(
              'relative z-50 flex max-h-[75vh] md:max-h-[90vh] flex-1 flex-col overflow-hidden border border-transparent bg-white dark:border-neutral-800 dark:bg-neutral-950 md:rounded-2xl !rounded-2xl',
              modalType === 'tweet' ? 'md:max-w-[43%]' : modalType === 'boost' ? 'md:max-w-[55%]' : 'md:max-w-[38%]',
              className,
            )}
